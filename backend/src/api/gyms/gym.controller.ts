...
export class GymController {
    ...
    async addUser(@Param('id') gymId: string, @Param('userId') userId: string): Promise<void> {
        return this.gymUserService.addUserToGym(userId, gymId);
    }

    @Delete(':id/users/:userId')
    async removeUser(@Param('id') gymId: string, @Param('userId') userId: string): Promise<void> {
